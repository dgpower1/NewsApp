...
class NewsTableViewCell: UITableViewCell {
    ...
    func configure(with viewModel: NewsTableViewCellViewModel) {
        ...
        if let data = viewModel.imageData {newsImageView.image = UIImage(data: data)}
        
        else if let url = viewModel.imageURL {URLSession.shared.dataTask(with: url) { [weak self] data, _, error in
